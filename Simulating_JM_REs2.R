#' ###
#' Updated version of Simulating_JM_REs.R
#' In this, hopefully the implementation of random slope is correct
#' (Implementing time dependence on random slope.)
#' ###
...
length(which(survtime < max(time)))/length(survtime)
...
simstudy$results <- parallel::mclapply(simstudy$data, joint_fit,
                                       mc.preschedule = F, mc.cores = 3) # 53 minutes!
toc()


# Extract parameters & plot -----------------------------------------------

extract_parameters <- function(fit){
  Bl <- fit$coefficients$fixed$longitudinal$b1[-6]
...
ggsave("./JM-sims-plots/SampleSizeRE_redux.png")
